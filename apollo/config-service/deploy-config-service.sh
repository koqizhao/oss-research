...
deploy_file=$project_path/apollo-configservice/target/apollo-configservice-*-github.zip
...
deploy()
{
    ...
    scp deploy-cs.sh $server:$deploy_path
    ...
    ssh $server "cd $deploy_path; echo '$PASSWORD' | sudo -S sh deploy-cs.sh; rm deploy-cs.sh;"
