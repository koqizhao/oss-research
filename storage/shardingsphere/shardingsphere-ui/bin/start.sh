...
LOGS_DIR=${DEPLOY_DIR}/../logs/shardingsphere-ui
if [ ! -d ${LOGS_DIR} ]; then
    mkdir ${LOGS_DIR}
fi
...
JAVA_OPTS=" -server -Xmx1g -Xms512m -Xmn256m -Xss256k -XX:+DisableExplicitGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:LargePageSizeInBytes=128m -XX:+UseFastAccessorMethods -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 "
